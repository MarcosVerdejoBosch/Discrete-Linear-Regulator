...
reaches approximately 0.85 and 0.84\,V in the two modes.
...
12.47 and 12.48\,ms. The figure shows the first
...
electrically equivalent to the stated resistor. The complete
140\,ms sequence was cross-checked with a 1\,$\mu$s maximum
timestep and explicit transient error control (\texttt{trtol=1}),
against the nominal 2\,$\mu$s runs.
...
throughout the test. Calibration at an \(8\,\mathrm{V}\) source
and \(27\,^{\circ}\mathrm{C}\) gives mean outputs of
\(5.00001\,\mathrm{V}\) and \(3.30000\,\mathrm{V}\), with
...
\]
The resulting values are \(0.158\,\mathrm{mV/V}\) in the
\(5\,\mathrm{V}\) mode and \(0.318\,\mathrm{mV/V}\) in the
...
\]
The endpoint magnitudes are \(0.016\,\mathrm{mV/A}\) and
\(0.0017\,\mathrm{mV/A}\) for the \(5\,\mathrm{V}\) and
...
Repeating the two endpoint loads with the maximum time step reduced
from \(2\) to \(1\,\mu\mathrm{s}\) changes their mean outputs
by less than \(0.14\,\mu\mathrm{V}\). This numerical check does
not establish comparable hardware accuracy or thermal performance.
...
0.06\,dB and 1.2\,degrees. This single-frequency
...
\(5.814\,\mathrm{V}\) in both modes, after the delay path removes
...
50\,$\Omega$ and 33\,$\Omega$ loads. In the 5\,V mode, fixed-input
holds confirm a 100\,mV loss from the initial output between
4.94 and 4.95\,V at the source, while enable and the main reference
...
measurement fixture must also establish a low-dissipation rest state. In the 3.3\,V
mode, UVLO still disables the regulator before dropout can be
extracted. These nominal-model results do not establish a hardware
