...
try {
    ...
    $stem = Join-Path $PSScriptRoot "SIM-09_loop_$Mode"
    ...
    Write-Host "Ejecutando SIM-09 $Mode con solver Alternate..."
    $process = Start-Process -FilePath $exe -ArgumentList ('-alt -b -Run "' + $stem + '.asc"') -WorkingDirectory $PSScriptRoot -WindowStyle Hidden -Wait -PassThru
    ...
    Write-Host 'Simulacion completa. El preset muestra el retorno de lazo con magnitud y fase.'
