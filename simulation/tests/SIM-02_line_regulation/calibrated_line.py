"""Prepare, inspect and plot full-circuit calibrated line-regulation tests."""
from pathlib import Path
import argparse,csv,hashlib,json,os,re,shutil
import numpy as np
HERE=Path(__file__).resolve().parent; ROOT=HERE.parents[2]
DEST=HERE/'results/calibrated_line';DEST.mkdir(exist_ok=True)
p=argparse.ArgumentParser();p.add_argument('action',choices=['prepare','inspect','plot','progress']);args=p.parse_args()
MODES=['5V','33V']
PWL='PWL(0 8 80m 8 80.1m 6 120m 6 120.1m 7 160m 7 160.1m 8 200m 8 200.1m 9 240m 9 240.1m 10 280m 10 280.1m 11 320m 11)'
def read_raw(path):
...
if args.action=='prepare':
    ...
    result={}
    for mode in MODES:
        stem=ROOT/f'simulation/ltspice/SIM-02_calibrated_line_{mode}'
        ...
        if 'Total elapsed time:' not in log or t[-1]<.31999:raise SystemExit('Incomplete run; no final results produced')
        ...
        for vin,a,end in [(8,.07,.08),(6,.11,.12),(7,.15,.16),(8,.19,.20),(9,.23,.24),(10,.27,.28),(11,.31,.32)]:
            ...
            r={'source_V':vin,'vout_avg_V':avg(y,a,end),'drift_mV':1000*(avg(y,a,end)-avg(y,a-.01,a)),
            ...
            rows.append(r)
        repeat=1e6*(rows[3]['vout_avg_V']-rows[0]['vout_avg_V']);assert abs(repeat)<1
        signed=1000*(rows[-1]['vout_avg_V']-rows[1]['vout_avg_V'])/5
        target=5 if mode=='5V' else 3.3
        result[mode]={'all_steps_completed':True,'results':rows,'signed_slope_mV_per_V':signed,'repeat_8V_difference_uV':repeat,
                      'maximum_nominal_error_percent':max(abs(r['vout_avg_V']-target)/target*100 for r in rows[1:])}
        manifest=[]
    ...
    for ax,mode,title in zip(axes,MODES,['(a) 5 V mode, 5 '+r'$\Omega$'+' load','(b) 3.3 V mode, 3.3 '+r'$\Omega$'+' load']):
        assert result[mode]['all_steps_completed'];rows=result[mode]['results'][1:]
        ax.plot([r['source_V'] for r in rows],[r['vout_avg_V'] for r in rows],color='#0072B2',marker='o',markersize=3)
        ax.set_xlabel('Source voltage (V)');ax.set_ylabel('Mean output voltage (V)');ax.set_xticks(range(6,12))
        ax.yaxis.set_major_formatter(ScalarFormatter(useOffset=False));ax.set_title(title,fontsize=9,loc='left')
    for ext in ['pdf','svg','png']:fig.savefig(ROOT/f'docs/figures/SIM-02_calibrated_line.{ext}',dpi=300)
    plt.close(fig)
    (DEST/'figure_manifest.json').write_text(json.dumps({'source':'metrics.json','sha256':hashlib.sha256((DEST/'metrics.json').read_bytes()).hexdigest(),'matplotlib':matplotlib.__version__,'numpy':np.__version__,'processing':'Time-weighted final 10 ms means; first startup point excluded from plotted sweep; no smoothing or fitting.'},indent=2)+'\n')
