...
stem=ROOT/'simulation/ltspice/SIM-02_full_line_5V_changes'
...
assert t[-1]>=.15999 and np.all(np.diff(t)>=0)
def avg(y,a,b):
 mask=(t>a)&(t<b);x=np.r_[a,t[mask],b]
 return float(np.trapezoid(np.interp(x,t,y),x)/(b-a))
rows=[]
for vin,a,end in [(8,.07,.08),(10,.11,.12),(11,.15,.16)]:
...
dest=HERE/'results/full_circuit';result={'all_steps_completed':True,'results':rows}
