...
for mode,load,nom in [('5V',50,5),('33V',33,3.3)]:
 p=R/f'simulation/ltspice/SIM-02_cal_02_{mode}.asc';s=p.read_bytes().decode('cp1252').replace('\r\n','\n')
 ...
 s=s.replace('SYMATTR Value {VINTEST}','SYMATTR Value PWL(0 8 80m 8 120m 4)')
 s=s.replace('.tran 0 80m 0 2u','.tran 0 120m 0 2u')
 ...
 manifest.append(dict(mode=mode,load_ohm=load,nominal_A=.1,source=p.name,source_sha256=hashlib.sha256(p.read_bytes()).hexdigest(),file=q.name,sha256=hashlib.sha256(q.read_bytes()).hexdigest(),adjustments={'LTspice R65 / KiCad RV12':{'original_ohm':15000,'test_ohm':47000},'LTspice R70 / KiCad RV10':{'original_ohm':75000,'test_ohm':100000}},hardware_pot_ranges={'RV10_max_ohm':100000,'RV12_max_ohm':50000,'confirmed_by_user':True},source_ramp_V_per_ms=-.1,solver='Normal' if mode=='5V' else 'Alternate',trtol=1,max_step_us=2))
